        ...
        <div class="inline">
            <form method="POST" action="assign-role-functionary" id="formroleFunctionaryController">
                ...
                <div class="flex items-center mt-4">
                        <div class="">
                            <label for="{{__('Select_role')}}"
                                class="block mb-2 TextColor font-bold">{{__('Select_role')}}</label>
                        ...
                        <div class="ml-4">
                            <label for="{{__('Select_functionary')}}"
                                class="block mb-2 TextColor font-bold">{{__('Select_functionary')}}</label>
                        ...
                        <div class="ml-4">
                            <button type="submit" name="function" value="assign"
                                class="color text-white py-2 px-4 rounded-lg mt-8">
                                Asignar
                            </button>
                        </div>

                        <div class="ml-4">
                            <button type="submit" name="function" value="lock"
                                class="color text-white py-2 px-4 rounded-lg mt-8">
                                {{__('change_state')}}
                            </button>
                        </div>
                    ...
                    <div class="ml-4 mt-8  ">
                        <input type="text" placeholder="Buscar..." class="px-4 py-2 border rounded-md w-full max-w-xs"
                            id="search-input">
                    </div>
                </div>
                                ...
                                @if($user->name !== 'CA')
                                    <tr class="text-sm text-gray-700 odd:bg-white even:bg-[#D9D9D9]">
                                        ...
                                        <td class="px-1 py-2 border-b text-center">
                                            <input type="checkbox" name="user_check[]" value="{{ $user->id }}"
                                                class="user-checkbox w-4 h-4">
                                        </td>
                                        <td class="px-4 py-2 border-b text-center">
                                            {{ $user->regional ? $user->regional->rgn_nombre : 'No asignado' }}</td>
                                        <td class="px-4 py-2 border-b">{{ $user->name }}</td>
                                        <td class="px-4 py-2 border-b text-center">{{ $user->supplier_document }}</td>
                                        <td class="px-4 py-2 border-b text-cente">
                                            {{ optional($user->roles->first())->name ?? 'Sin rol' }}</td>
                                        <td class="px-4 py-2 border-b text-center">{{ $user->position}}</td>
                                        <td class="px-4 py-2 border-b text-center">
                                            {{ $user->lock ? 'bloqueado' : 'No bloqueado' }}</td>
                                    </tr>
                                @endif
                            @endforeach
                        </tbody>
