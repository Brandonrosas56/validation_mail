...
{
    // Estados de Ticket
    public const STATE_PENDING_FUNCTIONARY = 'PENDING_FUNCTIONARY';
    public const STATE_PENDING_CONTRACTOR = 'PENDING_CONTRACTOR';
    public const STATE_SUCCESS_FUNCTIONARY = 'SUCCESS_FUNCTIONARY';
    public const STATE_SUCCESS_CONTRACTOR = 'SUCCESS_CONTRACTOR';

    public const STATE_SUCCESS_CONTRACTOR_CLOSE = 'SUCCESS_CONTRACTOR_CLOSE';
    public const STATE_REJECTED_FUNCTIONARY = 'REJECTED_FUNCTIONARY';

    public const STATE_REJECTED_FUNCTIONARY_CLOSE = 'REJECTED_FUNCTIONARY_CLOSE';
    public const STATE_REJECTED_CONTRACTOR = 'REJECTED_CONTRACTOR';
    public const STATE_REVIEW_CLOSE = 'REVIEW_CLOSE';
    ...
    public function __construct(CreateAccount|ValidateAccount $account, string $state, GLPIService $GLPIService, AccountTicketService $accountTicketService)
    {
        ...
        try {
            ...
            switch ($this->state) {
                case self::TEMPLATE_PENDING_FUNCTIONARY_CREACION:
                    ...
                    $typeTicket = self::STATE_PENDING_FUNCTIONARY;
                    break;
                case self::TEMPLATE_PENDING_CONTRACTOR_CREACION:
                    ...
                    Log::info('Plantilla de pendiente enviada', ['ticket' => $response]);
                    $typeTicket = self::STATE_PENDING_CONTRACTOR;
                    break;
                case self::TEMPLATE_SUCCESS_FUNCTIONARY_CREACION:
                    ...
                    $typeTicket = self::STATE_SUCCESS_FUNCTIONARY;
                    break;
                case self::TEMPLATE_SUCCESS_CONTRACTOR_CREACION:
                    ...
                    $typeTicket = self::STATE_SUCCESS_CONTRACTOR;
                    break;
                case self::TEMPLATE_SUCCESS_CONTRACTOR_CREACION_CLOSE:
                    ...
                    Log::info('Plantilla de éxito para creación de contratista enviada', ['ticket' => $response]);
                    $typeTicket = self::STATE_SUCCESS_CONTRACTOR_CLOSE;
                    break;
                case self::TEMPLATE_REJECTED_CONTRACTOR_CREACION:
                    ...
                    $typeTicket = self::STATE_REJECTED_CONTRACTOR;
                    break;
                case self::TEMPLATE_REJECTED_FUNCTIONARY_CREACION:
                    ...
                    Log::info('Plantilla de rechazo para creación de funcionario enviada', ['ticket' => $response]);
                    $typeTicket = self::STATE_REJECTED_FUNCTIONARY;
                    break;
                ...
                case self::TEMPLATE_REJECTED_FUNCTIONARY_ACTIVACION:
                    ...
                    $typeTicket = self::STATE_REJECTED_FUNCTIONARY_CLOSE;
                    break;
                ...
                case self::TEMPLATE_REVIEW_ACTIVACION_CREATION:
                    $response = $this->GLPIService->createTicket($this->reviewActivationCreationTemplate());
                    Log::info('Plantilla de revisión para activación/creación enviada', ['ticket' => $response]);
                    $typeTicket = self::STATE_REVIEW_CLOSE;
                    break;
                default:
            ...
            if ($response && !isset($response['error'])) {
                ...
                $ticket = $this->accountTicketService->create(
                    $this->account,
                    $response,
                    $typeTicket
                );
                if (!$ticket) {
                    Log::error('Error al crear el ticket en la base de datos local', [
                        'account_id' => $this->account->id,
                        'ticket_info' => $response,
                        'type_ticket' => $typeTicket
                    ]);
                } else {
                    Log::info('Ticket creado exitosamente en la base de datos local', [
                ...
                Log::error('Error al crear el ticket en GLPI', [
                    'response' => $response,
                    'account_id' => $this->account->id,
                    'type_ticket' => $typeTicket
                ]);
    ...
    private function reviewActivationCreationTemplate(): array
    {
        $userInfo = $this->getUserInfo();
        Log::info('Información plantilla de rechazo para activación de contratista', $userInfo);
